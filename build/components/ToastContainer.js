...
import { POSITION, ACTION, RT_NAMESPACE, eventManager, falseOrDelay, isValidDelay, objectValues } from './../utils';
class ToastContainer extends Component {
    constructor() {
         ...
         */
        this.collection = {};
        this.isToastActive = id => this.state.toast.indexOf(id) !== -1;
    }
    componentDidMount() {
        eventManager
            .cancelEmit(ACTION.WILL_UNMOUNT)
            .on(ACTION.SHOW, (content, options) => this.ref ? this.buildToast(content, options) : null)
            .on(ACTION.CLEAR, id => !this.ref ? null : id == null ? this.clear() : this.removeToast(id))
        ...
        if (isValidElement(toastClose) || toastClose === false) {
            ...
            closeButton =
                this.props.closeButton &&
                    typeof this.props.closeButton !== 'boolean' ? (this.props.closeButton) : (React.createElement(CloseButton, null));
        }
    ...
    appendToast(options, content, staleToastId) {
        ...
        this.setState({
            toast: (updateId
                ? [...this.state.toast]
                : [...this.state.toast, id]).filter(id => id !== staleToastId)
        }, this.dispatchChange);
    }
    clear() {
        this.setState({ toast: [] });
    ...
    renderToast() {
        ...
        collection.forEach(toastId => {
            ...
            if (this.state.toast.indexOf(options.id) !== -1) {
                toastToRender[position].push(React.createElement(Toast, Object.assign({}, options, { isDocumentHidden: this.state.isDocumentHidden, key: `toast-${options.key}` }), content));
            }
            else {
        ...
        return Object.keys(toastToRender).map(position => {
            ...
            const props = {
                className: cx(`${RT_NAMESPACE}__toast-container`, `${RT_NAMESPACE}__toast-container--${position}`, { [`${RT_NAMESPACE}__toast-container--rtl`]: this.props.rtl }, this.parseClassName(className)),
                style: disablePointer
                    ? { ...style, pointerEvents: 'none' }
                    : { ...style }
            };
            return (React.createElement(TransitionGroup, Object.assign({}, props, { key: `container-${position}` }), toastToRender[position]));
        });
    }
    render() {
        return (React.createElement("div", { ref: node => (this.ref = node), className: `${RT_NAMESPACE}` }, this.renderToast()));
    }
}
