...
const toast = (content, options) => dispatchToast(content, mergeOptions(options, (options && options.type) || TYPE.DEFAULT));
/**
 * For each available type create a shortcut
 */
for (const t in TYPE) {
    if (TYPE[t] !== TYPE.DEFAULT) {
        toast[TYPE[t].toLowerCase()] = (content, options) => dispatchToast(content, mergeOptions(options, (options && options.type) || TYPE[t]));
    }
}
/**
...
toast.update = (toastId, options = {}) => {
    ...
    setTimeout(() => {
        ...
        if (toast) {
            ...
            const content = typeof nextOptions.render !== 'undefined'
                ? nextOptions.render
                : oldContent;
